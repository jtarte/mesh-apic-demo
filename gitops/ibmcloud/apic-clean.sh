...
apic products:delete --scope catalog -s $server -o $org -c $catalog cloud-frontend-product:2.0.0
apic products:delete --scope catalog -s $server -o $org -c $catalog cloud-frontend-product:1.0.0

apic draft-products:delete -s $server -o $org cloud-frontend-product:2.0.0
apic draft-products:delete -s $server -o $org cloud-frontend-product:1.0.0


apic draft-apis:delete -s $server -o $org cloud-frontend-svc:2.0.0
